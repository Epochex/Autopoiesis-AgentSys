...
| `flow_store` | 实时管道 | ClickHouse `netops.facts`,6180 万条转发会话的全量历史 |
...
采集账本因此是独立的数据源,`netops-arp-collect.timer` 每 5 分钟落一次。
...
python3 -m domains.network_rca.collect_arp \
  /data/autopoiesis-runtime/arp-snapshot.txt \
  --ledger /data/autopoiesis-runtime/l2-identity-history.jsonl
```
