...
`autopoiesis-facts-ingest.service` 常驻执行 `facts_ingest.py live` 管线。它通过 SSH 在 R230 上只读执行 `tail -n0 -F`，解析 FortiOS KV 日志，先把批次写入本地持久发送箱并发布到 Redpanda `autopoiesis.events.raw.v1`，随后批量归档到 ClickHouse `autopoiesis` 数据库。Redpanda 暂时不可用时批次保留并重试，ClickHouse 继续接收历史事实。服务日志只进入 journald。
