...
`SegmentedBM25Index` 的封段只处理小增量，不在写请求中执行全量压缩。压缩由 `IndexMaintenanceWorker` 定期检查或主动唤醒；该组件只负责调度和检查、尝试、成功、跳过、中止、失败、耗时指标。快照耐久性不在维护器中：`SegmentedBM25Index` 实现规范化 JSON、SHA-256 校验和原子替换，`VectorIndexLifecycle` 实现 SHA-256 文件清单、文件刷盘、原子 `CURRENT` 指针及上一代保留，并保存 `generation` 与 `applied_offset`。
...
向量索引使用十万条 128 维确定性合成高斯向量，更新一万条并删除一万条。压缩前有十一万条物理向量和九万条有效向量，压缩后回收两万条旧向量。查询 P95 从 1.34 ms 降到 0.98 ms，吞吐从 751.30 QPS 提升到 1252.19 QPS；Recall@10 从 0.898 变为 0.912。HNSW 重建耗时 19.79 秒，快照加载耗时 0.72 秒，重启后的查询结果逐项一致。
