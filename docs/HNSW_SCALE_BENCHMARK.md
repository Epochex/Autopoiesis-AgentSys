...
HNSW 在百万规模下显著降低查询延迟，但它不是免费的精确索引替代品。当前压测参数
...
784.13 MB；同规模 Flat 构建 0.31 秒，索引为 512.00 MB。已提交的百万级产物是首次
构建，`cache_hit=false` 且 `load_seconds=0.0`，没有测量百万索引的加载耗时；先前流传
的百万加载数字没有产物依据。单独的十万级生命周期产物记录了 0.7162 秒快照加载。
服务端仍应采用离线构建、持久化文件和索引代际切换，不能在请求路径重建。

## 测量口径

- 数据集：确定性生成并归一化的 float32 合成高斯向量，查询由生成向量集合中的向量加入 0.02 噪声得到。
...
十万规模下压测默认值 `efSearch=128` 达到 0.899 Recall@10，P95 约为 Flat 的七分之一。
