...
export const FAULT_CATALOG: FaultFamily[] = [
  ...
  },
  {
    ...
    rationale: [
      '采集停了,上面所有判断就都是在瞎猜,所以这一族排得靠前。重启一个已经挂掉的服务不可能更坏,清理也只删已经打包归档、而且确认送走了的旧日志。',
      'When collection stops, every verdict above is guesswork, which is why this ranks high. Restarting an already-failed service cannot make it worse, and cleanup only removes rotated logs already shipped.',
    ],
    playbook: [
      ...
      { risk: 'readonly', what: ['看日志盘还剩多少', 'How much room is left on the log volume'], command: 'df -h /data' },
      { risk: 'auto', what: ['重启挂掉的采集服务,然后回头确认日志确实又进来了', 'Restart the failed collector, then confirm events are flowing again'], command: 'systemctl restart <failed-unit> && sleep 10 && curl -s localhost:8026/api/healthz' },
