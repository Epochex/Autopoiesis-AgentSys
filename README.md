...
**一套内网态势感知的多智能体系统，跑在一个可迁移的自演化 Agent 核心（AgentSys）之上。** 演示界面读取 2026-06-16 至 2026-06-17 的 FortiGate/R230 留出集，以及最新记录停在 2026-07-22 的磁盘落地态势数据，用于根因分析、攻击面侦察与环境盲区点名；下面这层 AgentSys 把「连续运行数周的智能体会踩的坑」做成一套可测量、可回放、可演化的内核，覆盖记忆污染、上下文膨胀、技能干扰和经验沉淀，并以清晰的域接口暴露出来。全部数字出自 [`core/`](./core) + [`domains/`](./domains) 的确定性 Python 内核。

<p align="left">
<b>百万级向量索引</b>&nbsp;·&nbsp;<b>ClickHouse 51.9M 行，审计时已 18 天无新增</b>&nbsp;·&nbsp;<b>十万条持续变更索引</b>&nbsp;·&nbsp;<b>743 项确定性测试</b>&nbsp;·&nbsp;<b>2026-06-16 至 17 日生产日志留出集</b>
</p>

系统分四条线，界面按这四条线展开：**态势记录线**，读取留出集和磁盘落地记录，做关联、根因、侦察与检索；**对话排查线**，按故障问一句话，系统先跑只读检查再让模型基于命令输出推理，产出分级 runbook；**处置预案线**，界面展示只读步骤与需要人工审批的步骤；**后台演化线**，核验通过的回放轨迹进入记忆写入、反思、冲突消解和索引投影。效用驱逐需要显式配置容量预算，出厂配置下不执行。

```text
态势记录:  留出集/磁盘落地记录 → 关联/簇窗口 → 记忆召回(BM25/资产/关系, 可选 HNSW) → 上下文编译 → 技能调度 → 多智能体推理 → 验证 → 处置预案 + trace
...
系统的演示数据来自两条离线读取路径：根因分析读取 **2026-06-16 至 2026-06-17** 的 FortiGate/R230 留出集，态势条带读取最新记录停在 **2026-07-22** 的告警、建议与簇状态磁盘文件。ClickHouse 当前共 **51.9M 行**，审计时已连续 **18 天**没有新增。态势台把内网设备按厂商聚成设备星座，下钻子网后每台设备是可点选节点，威胁点亮、`AGENT 关联分析` 拉出关系。
...
![落地态势记录 · 处理链](./docs/assets/ui/live_situation_chain.png)
...
![能力 → 基准 → 指标来源映射](./docs/assets/ui/bench_capability_map.png)

- **内网根因分析 → 网络 RCA 自愈回放（离线）**：2026-06-16 至 17 日 R230 FortiGate 留出集在临时目录中完成 6 类事件 × 4 pass 的规则推理回放，取证 32 次，根因与引用核验逐条对上，记忆记录增加 19 条。
- **长期记忆/检索 → LongMemEval-500（结果文件）**：分段 BM25 的 recall@5 = **0.970**，与该评测的 BM25 词法上限 0.970 一致；0.906 是升级前的词袋匹配结果。网关失败时，界面明确切换为内置 fixture。
