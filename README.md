...
ClickHouse 是事实存储和查询入口，Redpanda 承担连续事件传递。采集服务先把待发送批次落到 R450 本地发送箱，再写 ClickHouse，然后向 Redpanda 发布；消息服务短暂不可用时，批次保留并按顺序重试。稳定事件标识让下游能够丢弃重试造成的重复消息。生产主题只接收当前真实事件，历史回灌数据保留给季节性基线、稀有故障覆盖、缺口补数和离线回归测试。
...
| 指标 | M | A1 | 配对结果 |
|---|---:|---:|---:|
| ACT 实例核实成功数 | 10/10 | 10/10 | McNemar mid-p=1.0 |
| 工具调用 | 1 个实例减少 1 次，9 个持平 | 基准 | 平均减少 0.1，Wilcoxon p=1.0 |
| 上下文 token | 相关记忆进入上下文 | 关闭记忆 | Hodges-Lehmann 差 +339.5，p=0.007812 |

当前小规模稳定网络中的结论是“成功率收益尚未检出”。M 与 A1 均为 10/10；M 平均少 0.1 次工具调用，9 个实例持平、1 个减少，配对检验 p=1.0。M 相对 A1 的上下文 Hodges-Lehmann 差为 +339.5 token，95% CI 为 91.0 至 671.5，p=0.007812。现有证据确认机制已执行，同时测得显著上下文成本，业务收益仍待更大且更难的复发案例验证。
...
每周 CI 运行固定 seed 的五臂配对测试，并由 [`tests_py/test_memory_effectiveness_suite.py`](./tests_py/test_memory_effectiveness_suite.py) 锁定四项判定边界。详细统计、面试表达与公开 benchmark 对接计划见 [记忆有效性测试](./docs/MEMORY_EFFECTIVENESS.md)。公开锚点按能力拆分：LongMemEval-V2 测长轨迹状态与流程记忆，MemoryAgentBench 测选择性遗忘，EvoMemBench 测跨回合执行经验迁移，ITBench 测企业 IT 任务，AIOpsLab 补可注入故障、动作回读和恢复结果。当前没有单一公开测试集同时覆盖内网设备证据、跨事件记忆、权限边界、负迁移和动作回读，因此内部配对集负责业务闭环，公开基准负责外部能力锚定。
