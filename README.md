...
实时接案目前有三条入口。事件处理服务从实时安全日志中形成告警，局域网广播和组播拒绝在检测阶段退出案件链；管理登录攻击按一分钟内失败次数与不同公网来源数形成一个持续事件；二层身份巡检只把已经通过当前 DHCP、ARP 和历史所有权交叉核验的高风险发现写入案件。每个入口都产生稳定来源标识、受管资产、时间范围和故障域，后台据此启动调查。地址冲突调查重新读取当前二层身份报告，管理登录调查精确查询同一设备和时间窗口内的安全事件，调查结论保存实际探针证据。
