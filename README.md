...
每个调查会话持久保存根因候选、探针状态、支持证据、反证和状态版本。开放式问题先执行四项高优先级只读探针，后续分析根据尚未解决的候选继续取证。工具超时只记录采集失败，不能排除候选；当前设备在当前事件时间窗内的决定性观测才能确认或否定候选。服务重启后从会话快照恢复同一竞争状态。唯一根因确认且其余候选全部排除后，系统自动生成故障档案；模型给出的开放式新根因保留为待验证假设，直到后续探针或处置回读补齐证据。
...
现有评测分别测量代码契约、固定案例行为、检索召回和索引性能，这些结果不作为业务链完成度。业务准入依据时间序列案件：实时事件创建持久化案件，延迟证据到达后继续调查，反证能够修正假设，服务重启后从检查点恢复，有记忆与无记忆在同一事件流上进行配对。新的轨迹评分器位于 [`core/eval/temporal_case_evaluation.py`](./core/eval/temporal_case_evaluation.py)；当前仅有契约测试，尚无可发布的线上案件结果。
...
| 时间序列案件 | 待线上轨迹接入 | 持久状态、延迟证据、反证、重启恢复与记忆消融 |
...
| 授权动作回读 | 审批、动作回执、结果回读与回退全部留痕 | 结论正确，真实修改缺少授权或结果回读 | 当前只读测试待扩展 |
...
- 当前只读测试不会给动作安全结论。动作安全需要审批、动作回执、结果回读、回退和不必要动作率共同进入测试。
