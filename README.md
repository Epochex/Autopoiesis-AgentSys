...
| 主机服务、链路和资源故障 | 多个候选原因使用当前只读观察相互区分 | 只剩一个已确认原因，或已定义检查全部未命中 | 生成一个可执行动作，或明确升级责任范围 |
...
5. 记忆收益只按同一案件的配对结果计算，要求根因不变或更准确、确认步骤减少、错误动作和未约束上下文不增加。

测试集按这五条组织：来源字段与案件归并位于 `tests_py/test_runtime_reader.py` 和 `tests_py/test_investigation_case.py`；案件决策、动作回读和配对收益位于 `tests_py/test_investigation_business_loop.py`；跨时间反证、重启恢复和记忆消融位于 `tests_py/test_temporal_case_evaluation.py`。固定案例用于锁定行为契约，部署验收还需要从当前数据流抽取真实案件，检查根因正确率、错误结案率、错误动作率、平均取证次数和动作回读成功率。
