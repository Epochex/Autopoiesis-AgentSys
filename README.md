...
系统分四条线，界面按这四条线展开：**在线态势线**——事件流进来，多智能体做关联、根因、侦察、检索，产出可核验的处置建议；**对话排查线**——按故障问一句话，系统先跑只读检查再让模型基于真实输出推理，产出分级 runbook；**处置闭环线**——只对已经坏掉的单台目标动手，改完回读、进观察期、指标回归就自动退回；**后台演化线**——核验通过的运行轨迹回流成记忆，经写入路由、冲突消解、效用驱逐沉淀为稳定策略，并投影进可增量维护的索引层。
