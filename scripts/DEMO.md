...
- 状态先显示“刚发现”，随后显示“写操作已保留”，摘要同时给出安全门理由。
...
- 哨兵从当前认证日志独立发现了事件。
- 页面 1 已经接收实时检测结果。
- 页面给出安全门理由：`203.0.113.77` 是 RFC 5737 演示保留地址，本次只有注入日志，没有真实连接可阻断。
- 写入真实防火墙会制造无效 ACL，并增加管理通道误封风险，因此候选封禁动作被保留为建议，防火墙配置保持不变。
...
- 顶部处理轨迹完整点亮“巡检发现 → 二次确认 → 安全门判定 → 记录并转人工”。
- “安全决策”卡显示候选动作、拒绝理由和“防火墙配置未变化”的最终结果。
- 状态明确显示“自动流程已结束，待人工处置”。

它说明：

- 页面跳转携带了事件对象，落点是对应记录。
- 这是一条完整结束的安全拒绝分支，页面以明确终态展示整条决策链。
